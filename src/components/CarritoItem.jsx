...
function CarritoItem({ item, aumentarCantidad, disminuirCantidad, eliminarDelCarrito }) {
  return (
    <div className="flex items-center gap-4 border-b border-black py-6">
      <img
        src={item.imagen}
        alt={item.nombre}
      ...
      <div className="flex-grow min-w-0">
        ...
        <h3 className="font-bold uppercase text-sm leading-tight">{item.nombre}</h3>
        <p className="text-xs text-gray-500 mt-0.5">Talle: {item.talla}</p>
        <p className="text-sm font-black mt-1">${item.precio.toLocaleString('es-AR')}</p>
      </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          ...
          className="w-8 h-8 border border-black flex items-center justify-center font-bold hover:bg-black hover:text-white transition-all duration-200"
        >
          −
        </button>
      ...
      <div className="text-right flex-shrink-0 min-w-[100px]">
        <p className="font-black">${(item.precio * item.cantidad).toLocaleString('es-AR')}</p>
        {/* Eliminar productos del carrito */}
        <button
          onClick={() => eliminarDelCarrito(item.id, item.talla)}
          className="text-xs text-gray-400 hover:text-black transition-colors uppercase tracking-wider mt-1"
        >
          Eliminar
        </button>
      </div>
